...
def extract_link(html_file):
    ...
    link_list = ['http://bt.neu6.edu.cn/' + str(x)[23:46] for x in soup.find_all(href=re.compile('^t'))][
                0:6]  # 更改':'后面的数字可以决定索引前N个torrent
    return link_list
...
def check_new_torrent(torrent_list):
    filename_list = [name for name in os.listdir('x://x/x/x')]  # 这里是torrent文件的保存路径
    new_torrent_list = []
...
def download_torrent(real_torrent_list):
    for torrent in real_torrent_list:
        file = requests.get(torrent[1], cookies=get_cookie())
        with open('x://x/x/x/' + torrent[0], 'wb') as f:
            f.write(file.content)
        os.startfile('x://x/x/x/' + torrent[0])  # 这里是torrent文件的保存路径与上面的路径保持一致
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if 'cookies.txt' in files and os.path.getsize('cookies.txt') != 0 and isLoginStatus() is True:
            ...
            login('用户ID', '密码')  # 次数输入六维空间账号ID和密码，账号ID可在个人资料中找到
